#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $AGY_HOME/skills."""
...
def _agy_home() -> str:
    return os.environ.get("AGY_HOME", os.path.expanduser("~/.agy"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "agy")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "agy-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_agy_home(), "skills")
