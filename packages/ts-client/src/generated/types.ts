...
export type DiffEntry = { relative_path: string; sd_path: SdPath; uuid: string | null; size: number; modified_at: string | null; content_id: string | null; kind: DiffEntryKind };
 ...
 */
export type FileOperationConflict = { source: SdPath; destination: SdPath; existing_kind: FileSystemEntryKind; existing_size: number | null };
 ...
 */
export type FileOperationPreflightInput = {
/**
 ...
 */
export type FileOperationPreflightIssue = { kind: FileOperationPreflightIssueKind; path: SdPath | null; message: string };

/**
 * Issue kind for file operation preflight checks.
 */
export type FileOperationPreflightIssueKind = "MissingSource" | "MissingDestinationParent" | "DestinationNotDirectory" | "UnsupportedPath" | "IoError";

/**
 * Structured result for Finder-style copy and move preflight checks.
 */
export type FileOperationPreflightOutput = { operation: FileOperationKind; sources: SdPathBatch; destination: SdPath; file_count: number; total_bytes: number; conflicts: FileOperationConflict[]; issues: FileOperationPreflightIssue[]; can_execute: boolean; requires_confirmation: boolean; supports_job_progress: boolean };
 ...
 */
export type FileSystemEntryKind = "File" | "Directory" | "Other";

/**
 * Output from validating a copy or move before starting a job.
 */
export type FileValidationActionOutput =
/**
 ...
 */
preflight?: FileOperationPreflightInput | null };
...
export type PathDiffInput = { source: SdPath; target: SdPath; strategy?: DiffStrategy; use_index_rules?: boolean };

export type PathDiffResult = { only_in_source: DiffEntry[]; only_in_target: DiffEntry[]; modified: DiffEntry[]; matched_count: number; copy_size: number; total_scanned: number };
...
export type ResolveEntryInput = { entry_uuid: string };

export type ResolveEntryOutput = { entry_uuid: string; entry_id: number };
...
export type LibraryQuery =
  ...
  |  { type: 'files.entry.resolve'; input: ResolveEntryInput; output: ResolveEntryOutput }
  |  { type: 'files.media_listing'; input: MediaListingInput; output: MediaListingOutput }
  |  { type: 'files.path_diff'; input: PathDiffInput; output: PathDiffResult }
...
export const WIRE_METHODS = {
  ...
  libraryQueries: {
    ...
    'files.entry.resolve': 'query:files.entry.resolve',
    'files.media_listing': 'query:files.media_listing',
    'files.path_diff': 'query:files.path_diff',
