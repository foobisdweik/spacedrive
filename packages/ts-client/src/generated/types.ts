...
export type GetFilesByTagInput = {
	tag_id: string;
...
export type GetSyncPartnersOutput = {
	partners: SyncPartnerInfo[];
	debug_info: SyncPartnersDebugInfo;
};
 ...
 */
export type JobLoggingConfigOutput = {
	enabled: boolean;
	log_directory: string;
	max_file_size: number;
	include_debug: boolean;
	log_ephemeral_jobs: boolean;
};
...
export type ListSourceItemsInput = {
	source_id: string;
	limit: number;
	offset: number;
};
 ...
 */
export type MemoryBreakdownStats = {
	arena: number;
	cache: number;
	registry: number;
	path_index_overhead: number;
	path_index_entries: number;
	entry_uuids_overhead: number;
	entry_uuids_entries: number;
	content_kinds_overhead: number;
	content_kinds_entries: number;
};
...
export type PathDiffInput = {
	source: SdPath;
	target: SdPath;
	strategy?: DiffStrategy;
	use_index_rules?: boolean;
};

export type PathDiffResult = {
	only_in_source: DiffEntry[];
	only_in_target: DiffEntry[];
	modified: DiffEntry[];
 ...
 */
export type ProxyPairingConfigOutput = {
	auto_accept_vouched: boolean;
	auto_vouch_to_all: boolean;
	vouch_signature_max_age: number;
	vouch_response_timeout: number;
	vouch_queue_retry_limit: number;
};
 ...
 */
export type ServiceConfigOutput = {
 ...
 */
export type SpacebotConfigOutput = {
	enabled: boolean;
	base_url: string;
...
export type SyncPartnerInfo = {
	device_uuid: string;
	device_name: string;
	is_paired: boolean;
};

export type SyncPartnersDebugInfo = {
	total_devices: number;
	sync_enabled_devices: number;
	paired_devices: number;
	final_sync_partners: number;
	device_details: DeviceDebugInfo[];
};
...
export type UpdateAdapterOutput = {
	adapter_id: string;
	old_version: string;
	new_version: string;
...
export type VouchingSession = {
	id: string;
	vouchee_device_id: string;
	vouchee_device_name: string;
	voucher_device_id: string;
	created_at: string;
	state: VouchingSessionState;
	vouches: VouchState[];
};
...
export type CoreAction =
	...
	| {
			type: 'network.pair.confirmProxy';
			input: PairConfirmProxyInput;
			output: PairConfirmProxyOutput;
	  }
	| {
	...
	| {
			type: 'network.pair.vouch';
			input: PairVouchInput;
			output: PairVouchOutput;
	  }
	| {
...
export type CoreQuery =
	...
	| {
			type: 'network.pair.vouching_session';
			input: VouchingSessionInput;
			output: VouchingSessionOutput;
	  }
	| {
			type: 'network.status';
...
export type LibraryQuery =
	| {
			...
			input: GetAdapterConfigInput;
			output: [AdapterConfigField];
	  }
	| {type: 'adapters.list'; input: ListAdaptersInput; output: [AdapterInfo]}
	| {
			type: 'config.library.get';
			input: GetLibraryConfigQueryInput;
			output: LibrarySettingsOutput;
	  }
	...
	| {
			type: 'files.by_tag';
			input: GetFilesByTagInput;
			output: GetFilesByTagOutput;
	  }
	| {
	...
	| {
			...
			input: CopyMetadataQueryInput;
			output: CopyMetadataOutput;
	  }
	...
	| {
			...
			output: RedundancySummaryOutput;
	  }
